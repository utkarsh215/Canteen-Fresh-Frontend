...
export default function MyOrders(){
                    ...
                    <div>
                        <div className="max-w-2xl mx-auto bg-slate-100 border border-yellow-400 rounded-lg sm:flex">
                            ...
                            <h5 className="font-normal text-gray-700">{item.time} || {item.date}</h5>
                                ...
                                {(!item.completed) && (!item.rejected) &&<h1 className="text-sm mb-1 block tracking-tight text-gray-700">Status: <span className="text-yellow-400">Processing</span></h1>}
                                {(item.completed) && (!item.rejected) &&<h1 className="text-sm mb-1 block tracking-tight text-gray-700">Status: <span className="text-green-600">Completed</span></h1>}
                                {(!item.completed) && (item.rejected) &&<h1 className="text-sm mb-1 block tracking-tight text-gray-700">Status: <span className="text-red-600">Rejected</span></h1>}
                                </div>
    ...
    <div class="mt-3 mb-3 w-full max-w-5xl px-4 md:px-5 lg-6 mx-auto">
        <h2 class="font-manrope font-bold text-4xl leading-10 text-black text-center max-md:text-4xl">Your Orders</h2>
      </div>
      <div className="my-5 grid md:mb-8 lg:grid-cols-1 xl:grid-cols-2 mx-9">
        {allOrders()}
      </div>
    </>)
}
